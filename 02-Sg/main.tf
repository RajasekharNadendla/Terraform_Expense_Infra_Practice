module "db"{
    source = "git::https://github.com/RajasekharNadendla/terraform-aws-sg-module.git"
...
module "web_alb"{
    ...
    project_name = var.project_name
    environment = var.environment
    sg_description = " SG for wen ALB Instances"
    vpc_id = data.aws_ssm_parameter.vpc_id.value
    common_tags = var.common_tags
    sg_name = "web_alb"
}
...
resource "aws_security_group_rule" "frontend_web_alb"{
    ...
    to_port = 80
    protocol = "tcp"
    source_security_group_id = module.web_alb.sg_id # where you are getting traffic from
